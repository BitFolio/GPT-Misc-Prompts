...
## User input
```
Format crypto trades to Excel file. Read each and every one of the txt files below. Parse and reformat each txt file data to a CSV file. The input file names are in the format of YYYY-MM-DD-trading-investing.txt. Each file contains the activity on a single day. Output a CSV file based on the data in the provided input files. Omit any analysis or commentary. Output the CSV file in the requested format.
...
- Be sure and parse each txt file. Do not skip any files.
```
